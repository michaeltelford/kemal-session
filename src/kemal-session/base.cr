...
module Kemal
  class Session
    ...
    def self.destroy(id : String)
      Kemal::Session.config.engine.destroy_session(id)
    end
    ...
    def destroy
      ...
      Kemal::Session.destroy(@id)
    end

    # Destroys all of the sessions stored in the storage engine
    #
    def self.destroy_all
      Kemal::Session.config.engine.destroy_all_sessions
    end
    ...
    #
    def self.all
      Kemal::Session.config.engine.all_sessions
    end
    ...
    def self.get(id : String)
      Kemal::Session.config.engine.get_session(id)
    end

    # :nodoc:
    # id is the session_id that were signing.
    def self.sign_value(id : String, secret = Session.config.secret)
      raise Kemal::Session::SecretRequiredException.new if secret == ""
    ...
    def self.create_cookie(session_id : String)
      HTTP::Cookie.new(
        ...
        expires: Time.local.to_utc + Session.config.timeout,
        http_only: true,
